...
| Phase 02 migration SQL | hand-written `prisma/migrations/20260910000000_phase02_auth_hardening/migration.sql` (small additive diff; `prisma migrate diff --from-migrations` was attempted first but requires a live shadow DB - unavailable, see error in git history of this file) | **PASS_STATIC** (reviewed by inspection, additive-only, no live apply) |
