...
test("Destination V4 composition and links",async({page})=>{await fulfill(page);await page.goto("/destinations/ha-noi");await expect(page.getByText("Hoàng thành")).toBeVisible();await expect(page.getByText("Một sự kiện")).toBeVisible();await expect(page.getByText("Một câu chuyện")).toBeVisible();await expect(page.getByRole("heading",{name:"Một hành trình"})).toBeVisible();await expect(page.getByRole("link",{name:/Hoàng thành/})).toHaveAttribute("href","/places/hoang-thanh");});
