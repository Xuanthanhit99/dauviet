...
export default function PublishedMedia({ id, summary, caption, kind = "image", locale = "vi" }: {
  id: string; summary?: MediaSummary; caption?: string; kind?: "image" | "audio"; locale?: "vi" | "en";
}) {
  const en = locale === "en";
  ...
  return <figure className="published-media">
    {available && kind === "image" && isImage ? <img src={publicUrl!} alt={asset?.altText ?? label ?? (en ? "No alternative description is supplied for this media." : "Media chưa có mô tả thay thế.")} onError={() => setFailed(true)} />
      : available && kind === "audio" && isAudio ? <audio controls preload="none" src={publicUrl!} aria-label={label ?? (en ? "Story audio" : "Âm thanh câu chuyện")} onError={() => setFailed(true)} />
      : <p className="published-media-fallback" role="status">{loading ? (en ? "Loading media…" : "Đang tải media…") : (en ? "Media is unavailable or lacks the rights / provenance required for display." : "Media chưa khả dụng hoặc chưa đủ thông tin quyền sử dụng / nguồn gốc để hiển thị.")}</p>}
    <figcaption>
      {label && <p>{label}</p>}
      {media && <div className="published-media-disclosure"><strong>{media.type}</strong>{media.isHistorical && <span>Historical media</span>}{media.isAiGenerated && <span>AI / Reconstruction{media.aiDisclosure ? `: ${media.aiDisclosure}` : ""}</span>}{!media.isAiGenerated && media.aiDisclosure && <span>{media.aiDisclosure}</span>}{media.type === "RECONSTRUCTION" && !media.isAiGenerated && <span>{en ? "Reconstruction — not documentary photography." : "Tái dựng — không phải ảnh tư liệu."}</span>}</div>}
      {asset && <div className="published-media-provenance">
        {asset.attributionText && <p>{asset.attributionText}</p>}
        {asset.creatorName && <p>{en ? "Creator" : "Tác giả"}: {asset.creatorName}</p>}
        {asset.rightsHolder && <p>{en ? "Rights holder" : "Chủ thể quyền"}: {asset.rightsHolder}</p>}
        <p>{en ? "Rights" : "Quyền sử dụng"}: {asset.license ?? asset.rightsStatus ?? (en ? "Not supplied" : "Chưa được cung cấp")}</p>
        <p>{en ? "Provenance" : "Nguồn gốc"}: {asset.provenanceNote ?? (en ? "No provenance description supplied." : "Chưa có mô tả nguồn gốc.")}</p>
        {asset.captureYear != null && <p>{en ? "Capture year" : "Năm ghi nhận"}: {asset.captureYear}</p>}
      </div>}
      {media?.isAiGenerated && <p>{en ? "AI content is not documentary evidence." : "Nội dung AI không phải bằng chứng tư liệu."}</p>}
      {kind === "audio" && available && <p>{en ? "No transcript is supplied." : "Chưa có bản chép lời đi kèm."}</p>}
    </figcaption>
  </figure>;
}
